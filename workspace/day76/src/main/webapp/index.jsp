...
<body>
	...
	<!-- 우리가 action에 경로를 어떻게 작성해야 서블릿으로 요청을 보낼 수 있을까?
		현재 페이지는 jsp 파일이고 자바 파일인 서블릿으로 요청을 보내기 위해서는
		서블릿에 어노테이션으로 등록된 경로를 작성하면 된다
		주의할 점 : action = "/hello"로 작성하면 contextPath가 사라지게 되므로 오류가 발생한다
		상대경로로 작성하거나 ./를 제거하고 작성한다
	 -->
		<input type="text" name="userName" placeholder="이름입력">
		<button>전송</button>
		<!-- button 태그는 기본타입이 submit이다
		submit 버튼은 현재 form태그의 action에 작성된 경로로 요청(reqeust)를 보낸다
